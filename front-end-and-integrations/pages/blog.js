  ...
  useEffect(()=>{
      ...
      getTrendingBlogData();
        ...
        const [category, setcategory] = useState();
            ...
            //confirm admin cookies to show post ID
  ...
  return (
   ...
   <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-4">
    {allSearchPosts.length > 0 ? searchPosts.map((posts)=> (
            ...
            <img src="images/calendar.png" width="25" style={{display:"inline-block"}}/>
            <div className='text-[90%] text-[#ccc] ml-[0.2cm]' style={{display:"inline-block"}}>{(new Date(posts.date).toDateString())}</div>
            {auth ? (<div className='mt-[0.1cm] text-[95%] text-[#aaa] font-[500]'>Post ID: {posts.id}</div>) : (<span></span>)}
            </div>
...
<button className='px-[0.3cm] py-[0.1cm] bg-[#000] text-[80%] rounded-md text-[#fff] m-[0.2cm] cursor-default' style={{boxShadow:"1px 1px 2px 2px #502"}}>BNB Smart Chain</button>
