              ...
              const unstakeTokens = async () => {
                ...
                const calculateRewards = async () => {
                  if(isConnected){
                   ...
                   const stakeContractWriteSettings = new Contract(stakeContractAddress, stakeContractABI, signer)
                   try {
                    const calculaterewards = await stakeContractWriteSettings.calcReward(tokenB)
                  } catch (error) {
                    console.log(error)
                    setLoading(false)
    ...
    return (
       ...
       {stakeOption ?
        ...
        <form>
         ...
         <input style={{display:"inline-block"}} className="w-[25%] float-right text-right bg-[#eee] outline-none text-[#000] placeholder-[#502]" type="text" id="tokenAamount" name="tokenAamount" onChange={(e) => setTokenAamount(e.target.value)} placeholder='0' />
         </div>
         </div>
        </form>
        </div>
         ...
         <div className='p-[0.5cm] bg-[#000] text-[#fff] rounded-md'>
          ...
          <div className="mb-[0.3cm]"><button className="px-[0.1cm] py-[0.1cm] bg-[#502] rounded-md w-[3.6cm] cursor-default" style={{boxShadow:"2px 2px 2px 2px #002"}}>Stake start date:</button> &nbsp;{tokenAduration ? (<span>{tokenAstartDate}</span>) : (<span>Nil</span>)}</div>
        ...
        <div className="mt-[0.5cm] bg-[#111] p-[0.5cm] rounded-xl " style={{boxShadow:"2px 2px 2px 2px #333"}}>
         ...
         <div className='text-[#222] clear-both font-[500]'>
          ...
          <span className="float-right">BNB Testnet</span>
         </div>
         <div className='mt-[1.5cm] clear-both font-[500]'>
        ...
        <button type="submit" className='text-center py-[0.3cm] bg-[#880] font-[500] text-[#000] w-[100%] mt-[0.5cm] rounded-md generalbutton4 cursor-pointer' onClick={(e) => {e.preventDefault();calculateRewards(tokenB)}}>Calculate Rewards</button>
