...
export default function SwapSection() { 
           ...
           useEffect(()=>{
            const getTheData = async() => {
              ...
              try {
                ...
                const parseBNBbalance = parseFloat(formatUnits(BNBbalance, 18)).toFixed(5);
                console.log(parseBNBbalance.toString())
                setuserBNBBalance(parseBNBbalance.toString())
            ...
            getTheData();  
           }, [userBNBBalance, userUSDTBalance, userDAIBalance, address, loading])
        ...
        const SwapTokens = async () => {
          if(isConnected){
           ...
           try {
            if (tokenA == "0" && tokenB != "0"){const swapBNBForTokens = await swapContractWriteSettings.swapEthForTokens(tokenB, {value:parseUnits(tokenAamount, 18)})}
            else if (tokenA == "1" && tokenB == "0"){
              const swapUSDTforBNB = await swapContractWriteSettings.swapTokensForEth(tokenA, parseUnits(tokenAamount, 18))
            }
            else if (tokenA == "2" && tokenB == "0"){
              const swapDAIforBNB = await swapContractWriteSettings.swapTokensForEth(tokenA, parseUnits(tokenAamount, 18))
            }
        ...
        //But we will first approve for tokens other than BNB
    ...
    return (
        ...
        <div className="swapdiv bg-[#111] p-[0.5cm] rounded-xl " style={{boxShadow:"2px 2px 2px 2px #333"}}>
         ...
         <span className='text-[#000] float-left'>Bal: ≈ {tokenA == "0" && (<span>{userBNBBalance}</span>)} {tokenA == "1" && (<span>{userUSDTBalance}</span>)} {tokenA == "2" && (<span>{userDAIBalance}</span>)}</span>
        ...
        <div className="swapdiv bg-[#111] p-[0.5cm] rounded-xl" style={{boxShadow:"2px 2px 2px 2px #333"}}>
         ...
         <select className="float-left outline-none bg-[#502] text-[#fff] p-[0.1cm]" onChange={(e) => setTokenB(e.target.value)}>
          ...
          <span className='float-right'>BNB Chain</span>
         </div>
         <div className='mt-[1.5cm] clear-both font-[500]'>
         <span className='text-[#000] float-left'>Bal: ≈ {tokenB == "0" && (<span>{userBNBBalance}</span>)} {tokenB == "1" && (<span>{userUSDTBalance}</span>)} {tokenB == "2" && (<span>{userDAIBalance}</span>)}</span>
         </div>
         </div>
        </div>
