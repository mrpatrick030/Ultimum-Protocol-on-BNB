...
export default function DaoStakingAndNFT ({stakedTokenCapital, BNBamount, setBNBamount, buyULT, showApproveStakeButton, showStakeButton, approveStakingContractFromTokenContract, stakeULTtokensToMint, mintNFT, userULTBalance, userNFTBalance, userBNBBalance}) {
    return (
       <div>
        ...
        <div className="grid-cols-1 bg-[#111] p-[0.5cm] rounded-xl" style={{boxShadow:"2px 2px 2px 2px #333"}}>
         ...
         <div className='text-[#222] font-[500] clear-both'><span className="float-left">ULT bal</span><span className='float-right'>BNB amount</span></div>
         ...
         <input style={{display:"inline-block"}} className="w-[30%] float-right text-[120%] text-right bg-[#eee] outline-none text-[#000] placeholder-[#000]" type="text" id="BNBamount" name="BNBamount" onChange={(e) => setBNBamount(e.target.value)} placeholder='0' />
         </div>
         </div>
         <button type="submit" className='text-center py-[0.3cm] bg-[#502] font-[500] text-[#fff] w-[100%] mt-[0.5cm] rounded-md generalbutton cursor-pointer' onClick={(e) => {e.preventDefault();buyULT(BNBamount)}}>Buy ULT with BNB</button>
         {userBNBBalance == 0 && (<div className='text-center my-[0.5cm] lg:font-[500]'>You don't have enough BNB for gas fees! <span><img src="images/disappointment.png" width="20" className='ml-[0.2cm]' style={{display:"inline-block"}} /></span></div>)}
