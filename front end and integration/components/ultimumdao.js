...
export default function UltimumDAOSection({displayComponent}) {
      ...
      // lets read data for the DAO section using inbuilt functions and abi related read functions
       ...
       useEffect(()=>{
        ...
        getTheData();  
       }, [displayComponent, isConnected, userETHBalance, userULTBalance, userNFTBalance, stakedTokenCapital, address, showDAO, loading])
    ...
    const buyULT = async () => {
      if(isConnected){
       ...
       try {
        const buyult = await treasuryContractWriteSettings.buyTokens({value:parseUnits(ETHamount, 18)})
       } catch (error) {
        console.log(error)
        setLoading(false)
       }
